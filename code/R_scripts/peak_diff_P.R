...
data <- read.table('enhancer_P.tag_count.txt',sep="\t",header=T,fill=T,quote=NULL,comment.char="")
...
write.table(data2,sep="\t",file="peak_diff_P.all.txt",quote=F,col.names=T,row.names=F)

data2 <- data2[(data2$FC>=4) & (data2$pvalue<=0.0001),]
write.table(data2,sep="\t",file="peak_diff_P.filter.txt",quote=F,col.names=T,row.names=F)

#data3 <- data2[data2$Annotation %like% "Intergenic",]
#write.table(data3,sep="\t",file="peak_diff_P.intergenic.txt",quote=F,col.names=T,row.names=F)

#data4 <- data2[data2$Annotation %like% "intron",]
#write.table(data4,sep="\t",file="peak_diff_P.intron.txt",quote=F,col.names=T,row.names=F)

#data5 <- rbind(data3,data4)
#write.table(data5,sep="\t",file="peak_diff_P.intergenic_intron.txt",quote=F,col.names=T,row.names=F)
