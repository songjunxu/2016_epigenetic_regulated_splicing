...
setEPS()
postscript("exon-3kb.eps",width=12,height=6)

#cluster8
data <- read.table('cluster8-3kb.tag_count.txt',sep="\t",header=T,fill=T,quote=NULL,comment.char="")
...
#cluster14
data <- read.table('cluster14-3kb.tag_count.txt',sep="\t",header=T,fill=T,quote=NULL,comment.char="")
data$type <- sub(".*_", "", data$PeakID)
#statistics
labs <- c("promoter","variant","altDonor","canonical","altAcceptor","polyA")
for (i in 1:6){
...
#cluster19
data <- read.table('cluster19-3kb.tag_count.txt',sep="\t",header=T,fill=T,quote=NULL,comment.char="")
data$type <- sub(".*_", "", data$PeakID)
#statistics
labs <- c("promoter","variant","altDonor","canonical","altAcceptor","polyA")
for (i in 1:6){
